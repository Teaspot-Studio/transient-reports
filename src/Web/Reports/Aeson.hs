...
--
-- > { "items": [], "pages": 0 }
module Web.Reports.Aeson
  ( defaultOptions
  , defaultNewtypeOptions
  , leaveTagOptions
  , exactPrefixOptions
  , exactSumPrefixOptions
...
-- | Options for aeson TH deriver that drops exact prefix for fields and constructor
-- tags and preserves came case (e.g. "mySuperPower" to "superPower").
--
exactSumPrefixOptions :: String -> String -> Options
exactSumPrefixOptions fieldPrefix tagPrefix = defaultOptions {
      fieldLabelModifier = headToLower . stripExactPrefix fieldPrefix
    , constructorTagModifier = headToLower . stripExactPrefix tagPrefix
    , sumEncoding = ObjectWithSingleField
    }
